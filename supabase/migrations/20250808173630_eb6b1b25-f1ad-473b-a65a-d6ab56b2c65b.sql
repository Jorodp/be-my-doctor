-- Relax booking validation: only prevent past slots, remove lead time restriction
...
BEGIN
  ...
  -- Compute end time (60 minutes)
  v_slot_end := p_slot_start + interval '60 minutes';

  -- Only block if the slot is in the past
  ...
  -- Conflict check in appointments schema
  IF EXISTS (
