-- Fix: ensure book_slot_v2 returns exactly one row (avoid 406 when Accept expects object)
-- Keep existing logic, just add explicit RETURN of the OUT params
...
begin
  ...
  -- Explicitly return exactly one row
  RETURN QUERY SELECT out_appointment_id, out_starts_at, out_ends_at, out_status;
  RETURN;
end;
$$;