-- Update book_slot_v2 to robustly map clinic IDs and avoid FK violations
...
BEGIN
  ...
  -- Try to resolve a valid clinic_management.clinics id
  ...
  IF v_target_clinic_id IS NULL THEN
    -- Map by name/address from public.clinics within the same doctor
  ...
  IF v_target_clinic_id IS NULL THEN
    RAISE EXCEPTION 'No se encontró clínica válida para el doctor; verifique configuración o sincronice clínicas';
  END IF;

  -- Validate ownership in clinic_management
  IF NOT EXISTS (
  ...
  -- Base availability (public tables used by UI) use p_clinic_id (public id)
  WITH day_info AS (
  ...
  -- Conflict check and insert in appointments schema explicitly using resolved clinic id
  IF EXISTS (
