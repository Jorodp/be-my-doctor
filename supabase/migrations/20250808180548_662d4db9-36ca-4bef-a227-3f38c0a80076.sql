-- Improve book_slot to map public.clinics id to the FK target clinics table
...
BEGIN
  ...
  -- Try to resolve a clinic id that matches the FK target (commonly clinic_management.clinics)
  ...
  IF v_target_clinic_id IS NULL THEN
    -- Try to map by name/address within the doctor's clinics
    ...
    IF v_pub_name IS NOT NULL OR v_pub_address IS NOT NULL THEN
      ...
      WHERE c.doctor_id = p_doctor_internal_id
        AND (c.name = v_pub_name OR c.address = v_pub_address)
      LIMIT 1;
    END IF;
  END IF;

  -- Validate clinic ownership with the best-known table
  IF v_target_clinic_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'La clínica % no pertenece al doctor %', COALESCE(v_target_clinic_id, p_clinic_id), p_doctor_internal_id;
    END IF;
  ELSE
  ...
  -- Base availability in public (UI derived from public.availabilities)
  WITH day_info AS (
  ...
  -- Extra/blocked exceptions in public
  ...
  -- Use target clinic id for conflicts and insert; if not found, fall back to p_clinic_id
