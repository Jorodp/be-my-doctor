-- Create secure RPCs for assistants to reschedule and cancel appointments
-- 1) Helper: (inline checks inside each function)

-- 2) Assistant reschedule function
...
BEGIN
  ...
  -- Permission: assistant must be assigned to the clinic OR doctor
  IF NOT (
    EXISTS (
      SELECT 1
      ...
      JOIN public.doctor_assistants da ON da.doctor_id = dp.id
      WHERE dp.user_id = v_doctor_user_id AND da.assistant_id = v_assistant_id
    )
  ) THEN
  ...
  -- Compute new end time (60 minutes)
...
BEGIN
  ...
  -- Update appointment as cancelled and append note
  UPDATE appointments.appointments ap
  SET 
    status = 'cancelled',
