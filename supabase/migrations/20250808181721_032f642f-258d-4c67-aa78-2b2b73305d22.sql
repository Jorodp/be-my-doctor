-- Create new RPC with explicit schema usage to avoid public.appointments errors
...
BEGIN
  ...
  -- Only block past slots
  IF p_slot_start < now() THEN
    RAISE EXCEPTION 'No se puede agendar en el pasado';
  END IF;

  -- Try to use the same id in clinic_management.clinics if present, else fall back
  SELECT id INTO v_target_clinic_id FROM clinic_management.clinics WHERE id = p_clinic_id LIMIT 1;
  v_target_clinic_id := COALESCE(v_target_clinic_id, p_clinic_id);

  -- Validate ownership in either table
  IF NOT EXISTS (
  ...
  -- Base availability (public tables used by UI)
  WITH day_info AS (
  ...
  -- Conflict check and insert in appointments schema explicitly
  IF EXISTS (
