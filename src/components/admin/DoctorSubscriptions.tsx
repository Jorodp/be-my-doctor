...
const DoctorSubscriptions = () => {
  ...
  const getMonthlyRevenue = () => {
    ...
    return subscriptions
      .filter(sub => sub.status === 'active')
      .reduce((total, sub) => {
        // Convertir pagos anuales a equivalente mensual
        const monthlyAmount = sub.plan === 'annual' ? sub.amount / 12 : sub.amount;
  ...
  const getAnnualRevenue = () => {
    ...
    return subscriptions
      .filter(sub => sub.status === 'active')
      .reduce((total, sub) => {
        // Convertir pagos mensuales a equivalente anual
        const annualAmount = sub.plan === 'monthly' ? sub.amount * 12 : sub.amount;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <Card className="border-green-200 bg-green-50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-green-800">Facturación Mensual</CardTitle>
            <Receipt className="h-4 w-4 text-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-800">
              {formatCurrency(getMonthlyRevenue(), 'MXN')}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card className="border-blue-200 bg-blue-50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-blue-800">Facturación Anual</CardTitle>
            <Receipt className="h-4 w-4 text-blue-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-800">
              {formatCurrency(getAnnualRevenue(), 'MXN')}
            </div>
            <p className="text-xs text-muted-foreground">
